...
#define HDMI_SWITCH_HDMI_RX
    ...
    int blackout_check_start(int argc, char *argv[]);
    int blackout_check_stop(int argc, char *argv[]);
#endif
...
int hdmi_rx_enter(void)
{
	...
	if(projector_get_some_sys_param(P_CEC_ONOFF) && hdmirx_cec_dev_active_status_get() == 0){
        ...
        ret = hdmirx_cec_device_scan(hdmirx_handle);
        if(ret < 0)
        {
            return 0;
        }
        hdmirx_cec_poweron_device(hdmirx_handle);

        hudi_cec_get_active_devices(hdmirx_handle, &laes, 200);
        hdmirx_cec_active_device_sys_param_save(&laes);
