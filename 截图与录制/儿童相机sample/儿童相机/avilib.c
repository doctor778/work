...
avi_t* AVI_open_output_file(char * filename)
{
   ...
   AVI->fdes = open(filename, O_RDWR|O_CREAT, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH));   //进入了
#endif
   if (AVI->fdes < 0)
   {
   ...
   AVI->pos  = HEADERBYTES; //2048
   AVI->mode = AVI_MODE_WRITE; /* open for writing */   //0
...
int AVI_close(avi_t *AVI)
{
   ...
   if(AVI->mode == AVI_MODE_WRITE){       //进入了
