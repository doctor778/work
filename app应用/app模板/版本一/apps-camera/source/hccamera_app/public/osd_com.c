...
{
    if (if_transp)
    { // set the backgroud to transparency, may only valid 32bit colors??
...
// void win_msgbox_msg_open(const char *str_msg, uint32_t timeout, msg_timeout_func timeout_func, void *user_data) // 改为了直接传递字符串
// {
...
static void win_msgbox_msg_btn_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_KEY)
    {
        ...
        if (btn_sel != -1)
        {
            ...
            if (msgbox_func)
                msgbox_func(btn_sel, msgbox_user_data); // msgbox_user_data没有使用
            m_msgbox_obj = NULL;
        }
    }
}
...
void win_msgbox_btn_open(lv_obj_t *parent, char *str_msg, user_msgbox_cb cb, void *user_data) // 被注释的  // user_msgbox_cb 处理消息函数 发消息（哪个按键按下 弹窗已关闭）
{
    lv_obj_t *obj_root;
    ...
    // int id = projector_get_some_sys_param(P_OSD_LANGUAGE);  // id 是什么意思  按键失灵
    ...
    static const char *btns[] = {"pass", "fail", ""}; // , "" 不能删
    ...
    //  lv_obj_set_style_text_font(label, LV_STATE_DEFAULT, 0);

    // lv_obj_t *btns_obj = lv_msgbox_get_btns(m_msgbox_obj);
    //  lv_obj_set_style_text_font(btns_obj, select_font[id], 0);
    //  lv_obj_set_style_text_font(btns_obj, LV_STATE_DEFAULT, 0);
