...
#define INTERVAL_FIRST_KEY 350 //350 // ms
#define INTERVAL_NORMAL_KEY 80 //80 // ms
...
static void *_key_task(void *arg)
{
	...
	while (1)
	{
		if (poll(pfd, input_cnt, -1) <= 0)//通过poll函数等待事件发生。
		{
			continue;
		}

		for (i = 0; i < input_cnt; i++)//当有事件发生时，循环遍历所有输入设备，检查是否有事件发生。
		{
...
void api_key_get_init()//初始化键获取的 API，并启动相应的线程以进行键获取任务
{
